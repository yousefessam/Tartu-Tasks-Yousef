# -*- coding: utf-8 -*-
"""
Created on Sat Jun  2 23:20:58 2018
...
def plot_act(i=1.0, actfunc=lambda x: x):
    ws = np.arange(-0.5, 0.5, 0.1)
    bs = np.arange(-0.5, 0.5, 0.1)
...
act = tf.sigmoid(tf.matmul(i, ww) + b)
act.eval(session=sess)

plot_act(1, tf.tanh)

#Using tanh in a neural net layer
act = tf.tanh(tf.matmul(i, ww) + b)
act.eval(session=sess)
...
# it takes care of the Vanishing and Exploding Gradient 
act = tf.nn.relu(tf.matmul(i, ww) + b)
act.eval(session=sess)