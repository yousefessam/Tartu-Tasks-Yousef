...
#Ignoring what's in y, it looks more like 2 clusters of data points rather than 3 #c=y colors the scatter plot based on y (target) 
...
#How much data are we throwing away? #Lets try and plot number of components versus explained variance ratio as a cumulative sum to find out 
...
#This is great so far. Now we have a reduced 64 dimension dataset, 
#generated with 64 principal components. 
#Each of these principal components 
#can explain some variation in the original dataset. The parameter components_ of the 
...
#The principal components are vectors of the length = to the number of 
...
#In other words, we transformed the #64x64 pixel images into 8x8 images. 
...
# This is not bad at all, the image still looks pretty good but the finer details are missing, which is okay considering # we've reduced dimensionality by 64 times. 
...
plt.axis('off')
plt.imshow(img)
#Okay, so the array has 540 rows each of pixel 960x3. Let's reshape it into a format that PCA can understand. 
# 3840 = 1280x3
img_r = np.reshape(img, (960, 3840)) 
print(img_r.shape)

#img_rr = np.reshape(img, (540, 960,3)) 
#plt.imshow(img_rr)

# Great, now lets run RandomizedPCA with 64 components (8x8 pixels) and transform the image. 
ipca = PCA(svd_solver='randomized',n_components=64).fit(img_r) 
...
temp = np.uint8(temp)
print(temp.shape )
#reshaping 2880 back to the original 960x3 
